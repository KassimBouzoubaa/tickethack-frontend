function updateRemoveFromCartEventListener() {
  for (let i = 0; i < document.querySelectorAll('.delete').length; i++) {
    document.querySelectorAll('.delete')[i].addEventListener('click', function () {
      fetch(`https://tickethack-backend-three.vercel.app/cart/${this.id}`, { method: 'DELETE' })
...
// Get cart
fetch('https://tickethack-backend-three.vercel.app/cart')
  .then(response => response.json())
  .then(data => {
    if (data.result) {
...
document.querySelector('#purchase').addEventListener('click', function () {
  fetch('https://tickethack-backend-three.vercel.app/bookings', { method: 'PUT' })
    .then(response => response.json())
    .then(data => {
